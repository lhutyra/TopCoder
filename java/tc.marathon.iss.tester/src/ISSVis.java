 ...
 * m.child[0].child[1] is second SAW in starboard truss with BGA 1B
 * m.child[0].child[2] is third SAW in starboard truss with BGA 3A
 * m.child[0].child[3] is fourth SAW in starboard truss with BGA 3B
 *
 * m.child[1].child[0] is first SAW in port truss with BGA 2A
 * m.child[1].child[1] is second SAW in port truss with BGA 2B
 * m.child[1].child[2] is third SAW in port truss with BGA 4A
...
//System.out.println ( "zero" ) ;
        t1.mid.x = (t1.v1.x + t1.v2.x) * 0.5 ;
        t1.mid.y = (t1.v1.y + t1.v2.y) * 0.5 ;
        t1.mid.z = (t1.v1.z + t1.v2.z) * 0.5 ;
...
// case 3
// OK, now we have a point (t1.v23l) in the covering polygon and in
...
// exponential wrt number of sub-triangles (which is much better than
// fixed ratio subdivision schemes).
...
// ***********************************************************
        ...
        // if only one is "zero" then the resuting two point
        // sub-triangle will be tested for bisection on the recursion
        // this case is both zero...
// ***********************************************************
...
// ***********************************************************
            ...
            if ( level < temp_max_level - 1 )
            {
                ...
                double a1 = area * a12 - covered_area ;
                t2.value = a12*a31 ;
                t2.area = a1 ;
                t2.caseno = 11 ;
...
// ************************************************************
    {
        P str = new P ( ) ; // string top right
        P stl = new P ( ) ; // string top left
        P sbr = new P ( ) ; // string bottom right
        P sbl = new P ( ) ;
        P sstr = new P ( ) ; // substring of blocks top right
        P sstl = new P ( ) ;
        P ssbr = new P ( ) ;
        P ssbl = new P ( ) ;
        P bktr = new P ( ) ; // block of cells top right
        P bktl = new P ( ) ;
        P bkbr = new P ( ) ;
        P bkbl = new P ( ) ;
...
// gap width = 0.470meters / 6 = 0.078333meters
        ...
        final double block_w_range = (4000.0+gapw*5) / (4000.0+gapw*6) ;
        final double block_w_step = block_w_range / 5.0 ;
        final double block_w_gap = gapw / ( 4000.0 + gapw*6) ;

        double [] shadow_fraction = new double [41] ;
        sray.d = toSun ;
...
// ************************************************************
    {
        double cosine = 0 ;
        double [][] stringShadowFraction = new double [2][41] ;
...
// output is in 1A 2A order like almost all places
        double [] answer = new double [8 + 8*2*41 + 8*4] ;
...
//toSun.println ( ) ;
        calculateOneSAW ( 0 , 0 , 0 , m , toSun , proj , answer ) ; // 1A
        calculateOneSAW ( 1 , 1 , 0 , m , toSun , proj , answer ) ; // 2A
        calculateOneSAW ( 2 , 0 , 1 , m , toSun , proj , answer ) ; // 3A
        calculateOneSAW ( 3 , 1 , 1 , m , toSun , proj , answer ) ; // 4A
        calculateOneSAW ( 4 , 0 , 2 , m , toSun , proj , answer ) ; // 1B
        calculateOneSAW ( 5 , 1 , 2 , m , toSun , proj , answer ) ; // 2B
        calculateOneSAW ( 6 , 0 , 3 , m , toSun , proj , answer ) ; // 3B
        calculateOneSAW ( 7 , 1 , 3 , m , toSun , proj , answer ) ; // 4B
        step_number ++ ;
        return answer ;
    } ;
    // ************************************************************
...
// ************************************************************
    {
        ...
        {
            System.out.println("ERROR: unable to execute your solution." + e.getMessage());
            System.exit(0);
        }
...
// ************************************************************
    {
        ...
        loadModel();
        System.out.println("init formers");
        initFormatters();
